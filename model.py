...
class Attention(nn.Module): ## this is where the attention module is actually implemented, so elegant... so simple...
    def __init__(
        self,
        dim,
        heads = 8,
        ...
        super().__init__()
        inner_dim = dim_head * heads # this is used to set the dimensionality of the query, key and value projections
        self.heads = heads
        self.scale = dim_head ** -0.5 ## defines the scaling factor to apply to the dot products before applying softmax. This helps in stabilizing the gradients during training by scaling down large values

        self.to_qkv = nn.Linear(dim, inner_dim * 3, bias = False) ## A linear layer that projects the input feature vector into queries, keys, and values. The output dimension is three times the inner_dim because it includes separate projections for queries, keys, and values concatenated together.
...
class RowColTransformer(nn.Module):
    ...
    def forward(self, x, x_cont=None, mask = None):
        print(f"shape of x (x_cat that is): {x.shape}; shape of x_cont: {x_cont.shape}")
