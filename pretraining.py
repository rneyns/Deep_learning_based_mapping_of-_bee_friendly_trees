# -*- coding: utf-8 -*-
"""
Created on Mon Oct  2 11:40:28 2023
...
from data_prep import DataSetCatCon
...
def SAINT_pretrain(model,cat_idxs,X_train,y_train,  DOY_train, ids_train, continuous_mean_std,opt,device):
    train_ds = DataSetCatCon(X_train, y_train, DOY_train, ids_train, cat_idxs, opt.task)#, continuous_mean_std)
    ...
    for epoch in range(opt.pretrain_epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if 'cutmix' in opt.pt_aug:
                ...
                _ , x_categ_enc_2, x_cont_enc_2, con_mask = embed_data_mask(x_categ_corr, x_cont_corr, model,vision_dset,DOY=DOY)           
            else:
                _ , x_categ_enc_2, x_cont_enc_2, con_mask = embed_data_mask(x_categ, x_cont, model,vision_dset,DOY=DOY)           
            ...
            if 'contrastive' in opt.pt_tasks:
                aug_features_1  = model.transformer(x_categ_enc, x_cont_enc, con_mask)
                aug_features_2 = model.transformer(x_categ_enc_2, x_cont_enc_2, con_mask)
