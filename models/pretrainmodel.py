...
class SAINT(nn.Module):
    def __init__(
        ...
        *,
        ...
        attn_dropout = 0.,
        ff_dropout = 0.,
        cont_embeddings = 'spatio-temporal',
        scalingfactor = 10,
        ...
        cat_mask_offset = F.pad(torch.Tensor([self.num_categories,4]).fill_(2).type(torch.int8), (1, 0), value = 0) 
    ...
    def forward(self, x_categ, x_cont): ##this is actually not used + so many other things in this script are not used --> would love to clean it up
        
        x = self.transformer(x_categ, x_cont, con_mask)
        con_outs = self.mlpfory(x[:,0,:])
        return  con_outs 